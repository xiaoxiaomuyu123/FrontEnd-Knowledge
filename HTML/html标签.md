...
如果是跨行合并，就写在最上面的单元格上；如果是跨列合并，就写在最左侧上。      
   
合并步骤：   
1. 先确定是跨行还是跨列
2. 找到目标单元格，写上合并方式=合并单元格的数量 `<td rowspan='2'><td>`
3. 删除多余的单元格  
...
- input 输入表单元素，input 是单标签元素 `用户名<input type='text'>`  
    
    - 根据 type 属性的不同来指定不同的控件类型： 
          
        - text 定义单行的输入字段，用户可以输入文本，默认宽度是 20 个字符
        - password 密码框
        - radio 单选按钮，要注意不同的单选按钮要有相同的 name 属性值才能实现多选一。name是表单元素的名字   
        ...
        - button 定义一个普通可点击的按钮，通过 value 值来定义按钮上面显示的文字。注意需要提交的表单就用 `<input type='button'>`。而不需要提交内容的按钮，比如搜索按钮什么的，就直接用button标签。`<button><button>`
        - file 定义一个可以上传文件的按钮。点击之后就会出现本地计算机上面的硬盘显示。
          
    - 根据 name 来区分不同的表单控件，单选按钮和复选框必须有 name 值，并且 name 值相同。      
    - value 表示表单元素的值，也就是在表单框内的默认提示文字。当input中的type是输入类型的时候类似于 ant-D 里面的 placeholder，用来提示用户填写的格式。当是多选框或复选框中的value时，在用户点击提交的时候，会被发送到后台，让服务器知道用户选择了哪些信息。      
...
- 无序列表 ul li  
类似页面左侧导航栏那样整齐的无序元素，用无序列表表示   
- 有序列表 ol li     
一般用于页面的排行榜
- 自定义列表    
页面下方的联系方式等内容用自定义列表实现，上面有一个标题，下面有三个字内容，针对上面的标题进行解释说明   
dl 中只能有 dt 和 dd 标签，且两者是兄弟关系
